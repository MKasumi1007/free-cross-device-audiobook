...
MLX_MODEL_REPOSITORY="models--mlx-community--Qwen3-TTS-12Hz-0.6B-Base-4bit"
...
safe_remove_runtime() {
  ...
  case "$target" in
    ...
    "$DATA_ROOT/mlx-runtime"|"$DATA_ROOT/mlx-runtime.next"|"$DATA_ROOT/mlx-runtime.previous")
      /bin/rm -rf "$target"
      ;;
...
if [[ ! -f "$RESOURCE_DIR/requirements-agent.lock" \
  || ! -f "$RESOURCE_DIR/model_self_test.py" \
  || ! -f "$RESOURCE_DIR/requirements-mlx.lock" \
  || ! -f "$RESOURCE_DIR/mlx_model_self_test.py" ]]; then
  fail "安装包不完整，请重新下载。"
fi
...
export AUDIOBOOK_MLX_PYTHON="$DATA_ROOT/mlx-runtime/bin/python"
export AUDIOBOOK_MLX_MODEL="mlx-community/Qwen3-TTS-12Hz-0.6B-Base-4bit"
export AUDIOBOOK_TTS_BACKEND="mlx"
export AUDIOBOOK_TTS_BATCH_SIZE="2"
...
build_mlx() {
  local next="$DATA_ROOT/mlx-runtime.next"
  safe_remove_runtime "$next"
  "$UV" venv --python 3.12 "$next"
  "$UV" pip sync --python "$next/bin/python" "$RESOURCE_DIR/requirements-mlx.lock"
  "$next/bin/python" -c \
    "import importlib.metadata, mlx, mlx_audio, soundfile; assert importlib.metadata.version('mlx').startswith('0.32.')"

  local formal_model="$DATA_ROOT/models/huggingface/hub/$MLX_MODEL_REPOSITORY"
  local legacy_model="$HOME/.cache/huggingface/hub/$MLX_MODEL_REPOSITORY"
  if [[ ! -d "$formal_model" && -d "$legacy_model" ]]; then
    echo "正在把已有 MLX 模型迁入正式目录（优先使用 APFS 克隆，不重复占用空间）。"
    if cp -cR "$legacy_model" "$formal_model" 2>/dev/null; then
      :
    else
      /usr/bin/ditto "$legacy_model" "$formal_model"
  ...
  if [[ "$SKIP_MODEL_TEST" -eq 0 ]]; then
    "$next/bin/python" "$RESOURCE_DIR/mlx_model_self_test.py" \
      --data-root "$DATA_ROOT" --ffmpeg "$FFMPEG"
  fi
}
...
swap_runtime() {
  ...
  /bin/mv "$next" "$current"
  case "$name" in
    agent-runtime) AGENT_RUNTIME_SWAPPED=1 ;;
    qwen-runtime) QWEN_RUNTIME_SWAPPED=1 ;;
    mlx-runtime) MLX_RUNTIME_SWAPPED=1 ;;
  esac
  SWAPPED=1
}
...
rollback_and_restart_agent() {
  launchctl bootout "gui/$(id -u)/io.github.mkasumi1007.audiobook-mac-agent" \
    >/dev/null 2>&1 || true
  /bin/sleep 1
  if [[ "$MLX_RUNTIME_SWAPPED" -eq 1 ]]; then rollback_runtime "mlx-runtime"; fi
...
if [[ "$ACTION" == "full" || "$ACTION" == "mlx" ]]; then
  build_mlx
fi
...
if [[ -d "$DATA_ROOT/mlx-runtime.next" ]]; then swap_runtime "mlx-runtime"; fi
...
for item in install.sh update.sh update-agent-when-idle.sh uninstall.sh repair.sh \
  model_self_test.py mlx_model_self_test.py requirements-agent.lock \
  requirements-qwen.lock requirements-mlx.lock; do
...
chmod 700 "$DATA_ROOT/installer/"*.sh \
  "$DATA_ROOT/installer/model_self_test.py" \
  "$DATA_ROOT/installer/mlx_model_self_test.py"
chmod 600 "$DATA_ROOT/installer/requirements-"*.lock

if [[ "$ACTION" == "full" || "$ACTION" == "runtime" \
  || "$ACTION" == "mlx" || "$ACTION" == "launch_agent" ]]; then
...
if [[ "$ACTION" != "model" && "$ACTION" != "qwen" && "$ACTION" != "mlx" ]]; then
  HEALTH_OK=0
...
if [[ "$ACTION" == "qwen" || "$ACTION" == "mlx" ]]; then
...
safe_remove_runtime "$DATA_ROOT/mlx-runtime.previous"
